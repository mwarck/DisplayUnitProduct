...
class woa_wqfsp {
    ...
    add_filter( 'woocommerce_quantity_input_args', array($this, 'add_quantity_args'),10,2);
	...
	if ( $product && $product->is_type( 'simple' ) && $product->is_purchasable() && $product->is_in_stock() && ! $product->is_sold_individually() ) {
		...
		//rewrite form code for add to cart button
      $id= $product->id;
		$html = '<form action="' . esc_url( $product->add_to_cart_url() ) .'"id="'.$id.'" class="cart" method="post" enctype="multipart/form-data">';
		$html .= woocommerce_quantity_input( array(), $product, false );
    $html .='<div><input type="text" readonly style="width:33%;" name="product_id" id="base_' .$id. '" value=""/><a>gramos</a></div>';
    $html .='<input type="hidden" name="calculus_'.$id. '"  value="' .$meta_unit. '"class="calculus"/>';
  ...
  /**
   * [add_quantity_args description]
   * addingthe id product to each quantity input value for j s
   * @param [type] $args    [description]
   * @param [type] $product [description]
   */
  public function add_quantity_args($args, $product){
    $id= $product->id;
    $args['name'] = "'qty_'.$id";
    return $args;
  }
